...
% if the number of stable roots is smaller than predetermined variables
if sum(logcon) < n1;
    warning('Too few stable roots: no stable solution');
    M = NaN; C = NaN;J0 = NaN;
    return;
    % if the number of stable roots is larger than predetermined variables
...
%check the rank of Zkt. If cond(Zkt) is very large matrix is close to
%singular
if cond(Zkt) > 1e+14; 
...
% 1) make an initial guess on c0 d0
%starting values
c0=1;
d0=-.3;
...
while DIFF > 0.00000001;
    ...
    % matrices A and C of  X(t) = A X(t-1) + C u(t)
    A=A0\A1;
    C=A0\[1,0,0;]';

% 2) compute matrix Sigma=E[X(t)*X(t)']
%if package Control System is not available, uncomment this line and
