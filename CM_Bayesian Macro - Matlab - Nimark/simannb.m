...
while 1
  ...
  for m=1:sa_nt %number of iterations before temperature reduction
    for j=1:sa_ns %for each iteration before the step reduction
      for h=1:sa_nargs %for each parameter separately
        ...
        %if the new guess for the parameter is out of bound, make another
        %one
        ...
        % evaluate objective function at xp (call it fp)
        ...
        if fp<=f %(fp and f are the negative of the likelihood)
          x=xp;
          f=fp;
          % increase the number of accepted draws
          sa_nacc=sa_nacc+1;
          nacp(h)=nacp(h)+1;
          nup=nup+1;
        ...
        else % function value increases, i.e. the likelihood in the proposal
            % is lower(accept the draw with certain probability)
          p=exp((f-fp)/sa_t);                              
          pp=rand(1,1);    %random number between 0 and 1
          if pp<p % accept the draw
            x=xp;
            f=fp;
            % increase the number f accepted draws
            sa_nacc=sa_nacc+1;
            nacp(h)=nacp(h)+1;
            % increase the number of downhill moves accepted
            %(we accepted a draw that decreased the likelihood)
            ndown=ndown+1;
          else % if reject, count it within rejections
            nrej=nrej+1;
          end
        end
      end
    end
% after sa_ns number of iterations, make the step reduction
    % adjust maximal step size vm
    c=ones(sa_nargs,1)*2;%??
    for i=1:sa_nargs %for each parameter
      ratio=nacp(i)/sa_ns;
      if ratio>0.6
